/**
 * Title: KnowWE-Plugin-d3web-basic
 * Contains all javascript functions concerning the KnowWE-Plugin-d3web-basic.
 */
...
/**
 * Namespace: KNOWWE.plugin.d3webbasic
...
/**
 * Namespace: KNOWWE.plugin.d3webbasic.actions
...
KNOWWE.plugin.d3webbasic.actions = function(){
    return {
        ...
        init : function(){
            ...
            if(_KS('.highlight-xcl-relation').length != 0){
                _KS('.highlight-xcl-relation').each(function( element ){
                    _KE.add('click', element, KNOWWE.plugin.d3webbasic.actions.highlightXCLRelation);
                });
            }  
            //add highlight rule
            if(_KS('.highlight-rule').length != 0){
                _KS('.highlight-rule').each(function( element ){
                    _KE.add('click', element, KNOWWE.plugin.d3webbasic.actions.highlightRule);
                });
            } 
        },        
        /**
        ...
        highlightXCLRelation : function( event ) {
            ...
            if (restore) {
                KNOWWE.plugin.d3webbasic.actions.updateNode(restore.className, rel.topic, null);
            }
            KNOWWE.plugin.d3webbasic.actions.highlightNode(rel.kdomid, rel.topic, rel.depth, rel.breadth, event);
        },
        /**
