...
public class Rdf2GoUtils {
    ...
    /**
     * Generates the tree of all classes that the concept belongs to.
     * The tree contains all class the the concept is 'rdf:type' of.
     * Further, if for two class in the tree 'A rdfs:subClassOf B' holds, then A is a successor of B in the generated tree.
     *
    ...
    /**
     * @see de.knowwe.rdf2go.utils.Rdf2GoUtils#getClassHierarchy(de.knowwe.rdf2go.Rdf2GoCore, org.ontoware.rdf2go.model.node.URI, String, String)
     *
     * @param core    the repository to work with
     * @param concept the concept for which the class tree should be generated
     * @param subClassRelation the relation that defines the subclass hierarchy (usually rdfs:subClassOf)
    ...
    public static PartialHierarchyTree<URI> getClassHierarchy(Rdf2GoCore core, URI concept, String subClassRelation, String typeRelation) {
        ...
        for (QueryRow queryRow : queryResultTable) {
            final Node c = queryRow.getValue("c");
            final URI classUri = c.asURI();
