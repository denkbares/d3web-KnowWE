...
public class StartProgressAction extends OperationAction {
	...
	public void execute(final UserActionContext context, Section<?> section, final LongOperation operation) throws IOException {
		...
		String progressID = context.getParameter("ProgressID");
		AjaxProgressListener progressListener = ProgressListenerManager.getInstance().getProgressListener(operation);
		while(progressListener == null || !progressListener.getId().equals(progressID)) {
			...
			progressListener = ProgressListenerManager.getInstance().getProgressListener(operation);
		}
	}
}
