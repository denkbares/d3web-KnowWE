...
Timeline._Band = function(timeline, bandInfo, index) {
    ...
    var jqDiv = jq$(this._div);
    
    jqDiv.bind("mousedown", jq$.proxy(this._onMouseDown, this));
    jqDiv.bind("mousemove", jq$.proxy(this._onMouseMove, this));
    jqDiv.bind("mouseup", 	jq$.proxy(this._onMouseUp, this));
    jqDiv.bind("mouseout", jq$.proxy(this._onMouseOut, this));
    jqDiv.bind("dblclick", 	jq$.proxy(this._onDblClick, this));
    ...
    if (mouseWheel === 'zoom' || mouseWheel === 'scroll' || this._zoomSteps) {
    	var scope = this;
    	/*jq$(this._div).bind("mousewheel", function(event) {
    			var evt = event.originalEvent;
    		  var now = new Date();
    		  now = now.getTime();

    		  if (!scope._lastScrollTime || ((now - scope._lastScrollTime) > 50)) {
    		    // limit 1 scroll per 200ms due to FF3 sending multiple events back to back
    		    scope._lastScrollTime = now;
    		    ...
    		    var loc = SimileAjax.DOM.getEventRelativeCoordinates(evt, scope._div);
    		    scope.scroll(delta > 0, loc.x, loc.y);
    		  }

    		  event.preventDefault();
    		  return false;
    	});*/
    	jq$(this._div).bind('mousewheel', function(event, delta, deltaX, deltaY) {
	  		  ...
	  		  if (!scope._lastScrollTime || ((now - scope._lastScrollTime) > 50)) {
	  		    ...
	  		    var loc = SimileAjax.DOM.getEventRelativeCoordinates(event.originalEvent, scope._div);
	  		    scope.scroll(delta > 0, loc.x, loc.y);
	  		  }
...
Timeline._Band.prototype.scroll = function(zoomIn, x, y) {
    // either scroll or zoom
    var mouseWheel = this._theme.mouseWheel;
    
    if (this._zoomSteps || mouseWheel === 'zoom') {
      this._timeline.zoom(zoomIn, x, y, this._div);
    }
    else if (mouseWheel === 'scroll') {
    	var move_amt = 50 * (zoomIn ? 1 : -1);
...
Timeline._Band.prototype._onMouseDown = function(event) {
    this.closeBubble();
...
Timeline._Band.prototype._onMouseMove = function(event) {
    if (this._dragging) {
...
Timeline._Band.prototype._onMouseUp = function(event) {
    this._dragging = false;
    this._keyboardInput.focus();
};

Timeline._Band.prototype._onMouseOut = function(event) {
    var coords = SimileAjax.DOM.getEventRelativeCoordinates(event, this._div);
    coords.x += this._viewOffset;
    if (coords.x < 0 || coords.x > this._div.offsetWidth ||
        coords.y < 0 || coords.y > this._div.offsetHeight) {
        this._dragging = false;
    }
};
...
Timeline._Band.prototype._onDblClick = function(event) {
    var coords = SimileAjax.DOM.getEventRelativeCoordinates(event, this._div);
...
Timeline._Band.prototype.closeBubble = function() {
	SimileAjax.Graphics.closeBubble();
};
