...
Timeline.OverviewEventPainter.prototype.paint = function() {
    ...
    var fragment = this._timeline.getDocument().createDocumentFragment();
    var iterator = eventSource.getEventReverseIterator(minDate, maxDate);
    while (iterator.hasNext()) {
        var evt = iterator.next();
        if (filterMatcher(evt)) {
            this.paintEvent(evt, metrics, this._params.theme, highlightMatcher(evt), fragment);
        }
    }
    this._eventLayer.appendChild(fragment);
...
Timeline.OverviewEventPainter.prototype.paintEvent = function(evt, metrics, theme, highlightIndex, fragment) {
    if (evt.isInstant()) {
        this.paintInstantEvent(evt, metrics, theme, highlightIndex, fragment);
    } else {
        this.paintDurationEvent(evt, metrics, theme, highlightIndex, fragment);
    }
};

Timeline.OverviewEventPainter.prototype.paintInstantEvent = function(evt, metrics, theme, highlightIndex, fragment) {
    ...
    var tickElmtData = this._paintEventTick(evt, startPixel, color, 100, metrics, theme, fragment);
...
Timeline.OverviewEventPainter.prototype._paintEventTick = function(
    evt, left, color, opacity, metrics, theme, fragment) {
