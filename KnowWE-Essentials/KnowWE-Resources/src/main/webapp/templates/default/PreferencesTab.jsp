...
<form action="<wiki:Link jsp='UserPreferences.jsp' format='url'><wiki:Param name='tab' value='prefs'/></wiki:Link>"
	...
	<div class="form-group">
		<label class="control-label form-col-20" for="prefSectionEditing"><fmt:message
				key="prefs.user.sectionediting" /></label>
	...
	<div class="form-group">
		...
		<select id="prefLayout" name="prefLayout" data-pref="Layout">
			<option value='fluid' ${prefs.Layout=='fluid' ? 'selected="selected"' : ''} ><fmt:message
					key="prefs.user.layout.fluid" /></option>
			<option value='fixed' ${prefs.Layout=='fixed' ? 'selected="selected"' : ''} ><fmt:message
					key="prefs.user.layout.fixed" /></option>
		</select>
		<select id="prefOrientation" name="prefOrientation" data-pref="Orientation">
			<option value='fav-left' ${prefs.Orientation=='fav-left' ? 'selected="selected"' : ''} ><fmt:message
					key="prefs.user.orientation.left" /></option>
			<option value='fav-right' ${prefs.Orientation=='fav-right' ? 'selected="selected"' : ''} ><fmt:message
		...
		<select id="prefDisplayMode" name="prefDisplayMode" data-pref="DisplayMode" onchange="updateBodyClass()">
			<option value='light-mode' ${prefs.DisplayMode=='light-mode' ? 'selected="selected"' : ''} > Light Mode
			</option>
			<option value='dark-mode' ${prefs.DisplayMode=='dark-mode' ? 'selected="selected"' : ''} > Dark Mode
			</option>
		</select>
		<script type="text/javascript">
          function updateBodyClass() {
            let mode = document.getElementById('prefDisplayMode').value;
