 ...
 * <p/>
 * ActionServlet provides an extensible set of actions that can be used to provide required functionality. These actions
 * can be used from client side to proceed e.g. within the interview or to deliver required information.
 * <p/>
 * New actions can be added easily by providing a class implementing the {@link Action} interface and adds it to the
 * package specified by the this.COMMAND_PACKAGE constant (or a sub-package). The command name is the name of the class,
 * or if it is in a sub-package, the class name preceded by the missing sub-package names (e.g. command name for class
 ...
 * </pre>
 * <p/>
 * Depending on the action, it may utilize information given by query parameters or the path suffix to produce the
 * desired outcome.</li>
 * <p/>
 ...
 * </pre>
 * <p/>
 * The action tag can be repeated multiple times. The example above is almost identical to the following url:
 * <p/>
 * <pre>
...
public abstract class AbstractActionServlet extends HttpServlet {
	...
	protected void doAction(UserActionContext context) throws IOException {
		...
		Action.Access access = action.requiredAccess();
