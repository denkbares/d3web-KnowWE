...
public class CalendarHandler extends AbstractTagHandler{
	...
	private static List<CalendarEntry> getAppointments(DateType[] intervall, String author, String web) {
		...
		for(String elem: articleMap.keySet()) {
			...
			for(java.util.Map.Entry<String, Section<Appointment>> mapEntry : found.entrySet()) {
				...
				s[0] = mapEntry.getValue().findChildOfType(AppointmentStartSymbol.class).findChildOfType(AppointmentDate.class);
				s[1] = mapEntry.getValue().findChildOfType(AppointmentStartSymbol.class).findChildOfType(AppointmentTime.class);
	...
	private static int search(List<CalendarEntry> apps, DateType dateType, boolean after) {
		...
		if (!after) {author = "ZZZZZZZZZZ";}
		CalendarEntry ce = new CalendarEntry(new String[] {dateType.getDate(), dateType.getTime(), author, ""});
		apps.add(ce);
	...
	private static String buttons(String topic, int week) {
		...
		buttons.append("<button title='" + cb.getString("KnowWE2-Plugin-CalendarHandler.previous") + "' onclick=link('" + topic + "','" + (week - 1) + "')>" +
		...
		buttons.append("<form name=date method=post action=Wiki.jsp?page=" + topic + "&manually#Calendar accept-charset=UTF-8> " +
						"<input type=text name=input value='" + day.getDate() + "' size='12' maxlength='10'>" +
						" <button title='" + cb.getString("KnowWE2-Plugin-CalendarHandler.goto") + "' type=submit name=submit onclick=gotoDate()>" +
		...
		if(!day.sameDay(today)) {
			...
			// "today"-button
			buttons.append("<input type=hidden name=today value='" + today.getDate() + "' size='12' maxlength='10'>" +
					" <button title='" + cb.getString("KnowWE2-Plugin-CalendarHandler.today") + "' type=submit name=submit onclick=gotoToday()>" +
		...
		buttons.append("<button title='" + cb.getString("KnowWE2-Plugin-CalendarHandler.next") + "' onclick=link('" + topic + "','" + (week + 1) + "')>" +
		...
		for(int i = 0; i < 7; i++) {
			...
			if(entry != null && entry.getDate().sameDay(d)) {
				...
				while(entry != null && entry.getDate().sameDay(d)) {
					...
					toHTML.append("<tr><td style=text-align:left><b>" + entry.getDate().getTime() + "</b> " +
								"(" + (entry.getAuthor()=="-NA-"?"<i>-NA-</i>":entry.getAuthor()) + ")<br>" +entry.getValue() + "</td></tr>\n");						
	...
	private String renderWeekWide(DateType[] intervall, List<CalendarEntry> appointments) {
		...
		int[] x = {0,0,0,0,0,0,0}; //status for each day in entries[]
				...
				if(entries[day].length > x[day]) {
					...
					if(entry.getDate().getHour() == h) {
						...
						while(entry.getDate().getHour() == h) {
							...
							toHTML.append("<tr style=text-align:left><td><b>" + entry.getDate().getTime() + "</b> " +
										"(" + (entry.getAuthor()=="-NA-"?"<i>-NA-</i>":entry.getAuthor()) + ")<br>" +entry.getValue() + "</td></tr>\n");						
	...
	private String renderList(List<CalendarEntry> appointments) {
		...
		toHTML.append("<tr><th width=100>" + cb.getString("KnowWE2-Plugin-CalendarHandler.date") + "</th><th width=50>" + cb.getString("KnowWE2-Plugin-CalendarHandler.time") + "</th><th width=50>" + cb.getString("KnowWE2-Plugin-CalendarHandler.person") + "</th><th>" + cb.getString("KnowWE2-Plugin-CalendarHandler.text") + "</th></tr>\n");
		
		if (appointments != null && appointments.size() > 0) {
			for(int i = 0; i < appointments.size(); i++) {
				CalendarEntry entry = appointments.get(i);	
				toHTML.append("<tr><td style=text-align:center>" + entry.getDate().getDate() + "</td>" +
							"<td style=text-align:center>" + entry.getDate().getTime() + "</td>" +
									"<td style=text-align:center>" + (entry.getAuthor()=="-NA-"?"<i>-NA-</i>":entry.getAuthor()) + "</td>" +
											"<td>" +entry.getValue() + "</td></tr>\n");
			}
		} else toHTML.append("<tr><td colspan=4 style=text-align:center><i>" + cb.getString("KnowWE2-Plugin-CalendarHandler.empty") + "</i></td></tr>");
