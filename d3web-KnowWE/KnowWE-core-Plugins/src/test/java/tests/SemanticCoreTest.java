...
public class SemanticCoreTest {
	...
	public void testNamespaceStuff() {
		...
		assertEquals("http://swrc.ontoware.org/ontology#", sc.expandNamespace("swrc"));
		//check out defaultnamespaces
		nspaces=sc.getDefaultNameSpaces();
		//check for all the default ns
		assertEquals("http://www.w3.org/1999/02/22-rdf-syntax-ns#", nspaces.get("rdf"));
		assertEquals("http://ki.informatik.uni-wuerzburg.de/d3web/we/knowwe.owl#", nspaces.get("ns"));
		assertEquals("http://www.w3.org/2002/07/owl#", nspaces.get("owl"));
		assertEquals("http://www.w3.org/2000/01/rdf-schema#", nspaces.get("rdfs"));
		...
		assertEquals(386, sc.getSparqlNamespaceShorts().length());
		//check if a namespace is correctly reduced
		assertEquals("swrc:",sc.reduceNamespace("http://swrc.ontoware.org/ontology#"));		
	}
