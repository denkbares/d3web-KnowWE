package de.d3web.we.renderer.applet;
...
public class OWLApplet extends JPrefuseApplet {

    /**
	 * 
	 */
	private static final long serialVersionUID = -8188953073034793465L;	
	...
	public void init() {
        ...
        String topic=getParameter("url");        
        JComponent graphview = demo(topic,"name");        
    ...
    public static JComponent demo(String datafile,String label) {
        ...
        if ( datafile == null ) {
           ...
           try {
        	   Logger.getLogger("OWL").log(Level.INFO,datafile);
                g = new GraphMLReader().readGraph(datafile);
                int n=g.getNodes().getTupleCount();
                int e=g.getEdges().getTupleCount();
                Logger.getLogger("OWL").log(Level.INFO,"n:"+n+" :"+e);
            } catch ( Exception e ) {
                e.printStackTrace();
                System.exit(1);
            }
