...
public class Utils {
	 ...
	 * to start with a Section containing a QuestionTreeAnswerID, followed by 
	 * a Section containing a QuestionID. After that it starts again with a
	 * QuestionTreeAnswerID and so forth.  If thats not the case, <tt>null</tt> 
	 * will be returned.
	 * Such a List of DashTreeElements can be created with 
	 * <tt>DashTreeElement.getDashTreeAncestors(Section s)</tt>, if <tt>s</tt> is
	 * the child Section of an answer in a valid DashTree.
	 */
	public static AbstractCondition createCondition(List<Section<? extends DashTreeElement>> ancestors) {
		
		List<AbstractCondition> simpleConds = new ArrayList<AbstractCondition>();
		
		for (int i = 0; i + 2 < ancestors.size(); i += 2) {
			AbstractCondition simpleCond = createSimpleCondition(ancestors.get(i), ancestors.get(i + 1));
			if (simpleCond != null) {
	 ...
	 */
	public static AbstractCondition createSimpleCondition(Section<? extends DashTreeElement> father, 
			Section<? extends DashTreeElement> grandFather) {
		
		Section<QuestionTreeAnswerID> answerSec = father.findSuccessor(new QuestionTreeAnswerID());		
		...
		if(numCondSec != null && q instanceof QuestionNum) {
			...
			AbstractCondition condNum = FindingToConditionBuilder.createCondNum(father.getArticle(), numCondSec, comp, d, (QuestionNum)q);
			return condNum;
		}
