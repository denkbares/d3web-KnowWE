...
public class QuestionLine extends DefaultAbstractKnowWEObjectType {
	...
	static class createQuestionHandler implements ReviseSubTreeHandler {
		...
		@Override
		public void reviseSubtree(KnowWEArticle article, Section sec) {
			...
			if (o != null) {
				KDOMError.storeError(sec, new ObjectAlreadyDefinedError(o
						.getClass().getSimpleName()));
			} else {
				QASet parent = findParent(sec, mgn);

				QuestionType questionType = QuestionTypeDeclaration.getQuestionType(qidSection);
				Question q = null;
				...
				if(q != null) {
					QuestionID.storeQuestion(qidSection, q);
				}
	...
	static class TypeDeclarationRenderer extends KnowWEDomRenderer {
		...
		public void render(KnowWEArticle article, Section sec,
			...
			QuestionType questionType = QuestionTypeDeclaration.getQuestionType(sec);
			if (questionType.equals(
					QuestionType.OC)) {
	...
	static class QuestionTypeDeclaration extends
		...
		public static QuestionType getQuestionType(
				Section<QuestionID> s) {
			Section<QuestionTypeDeclaration> typeSection = s.getFather().findSuccessor(new QuestionTypeDeclaration());
