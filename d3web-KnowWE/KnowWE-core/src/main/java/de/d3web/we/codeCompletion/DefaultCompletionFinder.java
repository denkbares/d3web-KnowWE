...
public class DefaultCompletionFinder extends AbstractCompletionFinder {
	...
	public CompletionFinding find(String name, String data) {
		...
		// return new
		// CompletionFinding(name,name.substring(pos+data.length()),data.length());
		// if(this.startsWithIgnoreCase(name,data))
		if (name.startsWith(data)) return new CompletionFinding(name,
				name.substring(data.length()), data.length());
		// }
		return null;
	}
