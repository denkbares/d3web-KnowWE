...
public class SearchEngine {
	...
	public Collection<Result> search(String query, KnowWEArticle article) {
		...
		if (options.contains(SearchOption.FUZZY)) queryString = ".*?" + queryString + ".*?";
		else queryString = "\\s*?" + queryString + "\\s*?";

		Pattern p;
		if (options.contains(SearchOption.CASE_INSENSITIVE)) p = Pattern.compile(queryString,
				Pattern.CASE_INSENSITIVE);
		else p = Pattern.compile(queryString);
	...
	private Collection<Result> search(Pattern query, Section section) {
		...
		if (query.matcher(section.getOriginalText()).matches()) {
			if (section.getChildren().size() == 0) results.add(new Result(query.pattern(),
					section.getArticle(), section));
		}
