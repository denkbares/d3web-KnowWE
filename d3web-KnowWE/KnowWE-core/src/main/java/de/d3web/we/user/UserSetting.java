...
public class UserSetting {
	...
	private final Map<String, Map<String, NodeFlagSetting>> topicSettings = new HashMap<String, Map<String, NodeFlagSetting>>();
	...
	public boolean quickEditIsInPre(String nodeID, String topic) {
		if (topicSettings.get(topic) == null) return false;

		if (topicSettings.get(topic).get(nodeID) == null) return false;

		return topicSettings.get(topic).get(nodeID).isInPre();
	}

	public void setQuickEditFlag(String nodeID, String topic, String inPre) {
		if (topicSettings.get(topic) == null) {
			...
			setting.setInPre(Boolean.parseBoolean(inPre));
