...
public class UpdateQuestions extends AbstractAction {
    ...
    public void execute(ActionContext context) throws IOException {
    	...
    	String newQuestionsSection = addQuestion(currentQuestionSection, questionText, questionType, answers);
	 ...
	 * @return [0] everything before the new questions/solutions 
	 * 			[1] everything after the new questions/solutions
	 */
	public static String[] getRightInsertPosition(String article, String sectionName) {
		...
		// if there is a Kopic Tag, insert Question-section directly after
			...
			surrounding[0] = article.substring(0, article.indexOf("<Kopic>") + 7) + LINE_SEPARATOR + startTag + LINE_SEPARATOR;
			surrounding[1] = LINE_SEPARATOR + endTag + article.substring(article.indexOf("<Kopic>") + 7);
	...
	public static String getCurrentSectionContent(String article, String sectionName) {
		...
		if (article.contains(startTag)
				&& article.contains(endTag)) {
			return article.substring(article.indexOf(startTag) + startTag.length(), article.indexOf(endTag));
		}
		return "";
	}
