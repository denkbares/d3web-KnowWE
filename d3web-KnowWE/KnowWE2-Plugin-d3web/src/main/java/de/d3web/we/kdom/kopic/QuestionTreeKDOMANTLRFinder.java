...
import de.d3web.we.kdom.decisionTree.QuestionLineKDOMBuilder;
import de.d3web.we.kdom.decisionTree.QuestionTreeANTLR;
import de.d3web.we.knowRep.KnowledgeRepresentationManager;

public class QuestionTreeKDOMANTLRFinder extends SectionFinder {

	private QuestionLineKDOMBuilder builder;
	private DecisionTree parser;

	public QuestionTreeKDOMANTLRFinder(KnowWEObjectType type) {
		super(type);
	}
	...
	public List<Section> lookForSections(Section tmpSection, Section father,
		...
		builder = new QuestionLineKDOMBuilder(tmpSection.getTopic(), idg);
		...
		Section root = Section.createExpandedSection(tmpSection.getOriginalText(), new QuestionTreeANTLR(), null, -1, tmpSection.getTopic(), null, null, idg);
		int offset = 0;
		while(revert.size() > 0) {
