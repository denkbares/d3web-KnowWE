...
public class SolutionsKDOMANTLRFinder extends SectionFinder {

	private DashTreeKDOMBuilder builder;
	private DashTree parser;

	public SolutionsKDOMANTLRFinder(KnowWEObjectType type) {
		super(type);
	}
	...
	public List<Section> lookForSections(Section tmpSection, Section father,
		...
		builder = new SolutionsKDOMBuilder(tmpSection.getTopic(), idg);
		...
		Section root = Section.createExpandedSection(tmpSection.getOriginalText(), new SolutionsTreeANTLR(), null, -1, tmpSection.getTopic(), null, null, idg);
		while(revert.size() > 0) {
