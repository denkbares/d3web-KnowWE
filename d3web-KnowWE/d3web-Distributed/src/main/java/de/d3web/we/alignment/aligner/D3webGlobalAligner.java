...
public class D3webGlobalAligner implements GlobalAligner<NamedObject> {
	...
	public List<GlobalAlignment> align(Term term, NamedObject object, String idString) {
		...
		// Boolean privat = (Boolean)
		// object.getProperties().getProperty(Property.PRIVATE);
		...
		for (AlignMethod method : methods) {
			...
			if (!(type instanceof NoAlignType)) {
				...
				if (object instanceof Solution) {
					GlobalAlignment globalAlignment1 = new GlobalAlignment(term,
							new IdentifiableInstance(idString, object.getId(),
									new SolutionIdentity()),
							SolutionIdentityAlignType.getInstance());
					GlobalAlignment globalAlignment2 = new GlobalAlignment(term,
							new IdentifiableInstance(idString, object.getId(),
									new NumericalIdentity()),
