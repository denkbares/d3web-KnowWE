...
public class OwlUploadHandler extends AbstractTagHandler {
	...
	public String getDescription(KnowWEUserContext user) {
		return KnowWEEnvironment.getInstance().getKwikiBundle(user).getString(
				"KnowWE.OwlUploadHandler.description");
	}
	...
	public String render(String topic, KnowWEUserContext user,
		...
		Object[][] fields = { {
				"KnowWE.knoffice.fields.owlfile", KEY_OWL,
				"file", 50, "xml" } };
		...
		html.append("<h3>" + rb.getString("KnowWE.OwlUploadHandler.header")
				+ "</h3>");

		html
				.append("<form enctype=\"multipart/form-data\"  method=\"POST\" action=\"KnowWEUpload\">");
		// normal fields
		...
		if (files != null) {
			...
			for (File f : files) {
				...
				html.append("<input type=\"hidden\" id=\"" + KEY_DELETE_OWL
						...
						+ f.getName() + "\" />");
				html.append("<input id='upload' type='submit' value='"
						+ rb.getString("KnowWE.button.erase")
		...
		html.append("<br /><p><input id='upload' type='submit' value='"
				+ rb.getString("KnowWE.button.upload")
