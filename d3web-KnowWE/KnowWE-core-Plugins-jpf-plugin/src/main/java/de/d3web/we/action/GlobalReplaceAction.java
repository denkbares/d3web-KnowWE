...
public class GlobalReplaceAction extends DeprecatedAbstractKnowWEAction {
	...
	public String perform(KnowWEParameterMap parameterMap) {
		...
		for (String string : replacementArray) {
			...
			if(art == null) {
				//TODO report ERROR
				return "<p class=\"error box\">" + rb.getString("KnowWE.renamingtool.msg.noarticle") + article + "</div>";
			}
		...
		for (Entry<Section, List<WordBasedRenameFinding>> entry : findingsPerSection.entrySet()) {
			...
			for (WordBasedRenameFinding finding : list) {
				...
				if(potentialMatch.equals(query)) {
					...
					String errorMsg = rb.getString("KnowWE.renamingtool.msg.error").replace("{0}", sec.getOriginalText());
		...
		return errors.toString() + "<p class=\"info box\">" + summary + rb.getString("KnowWE.renamingtool.msg.redirect") + "</p>";
	}
}
