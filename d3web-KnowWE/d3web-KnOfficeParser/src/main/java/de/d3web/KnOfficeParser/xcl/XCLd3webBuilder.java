...
public class XCLd3webBuilder implements KnOfficeParser, XCLBuilder {
	...
	public List<Message> addKnowledge(Reader r,
		...
		XCL parser = new XCL(tokens, this, new DefaultD3webParserErrorHandler(errors, file,
				"BasicLexer"), cb);
		try {
			parser.knowledge();
		}
	...
	public void finding(String weight) {
		...
		if (currentdiag != null) {
			if (weight.equals("--")) {
				XCLModel.insertXCLRelation(idom.getKnowledgeBase(), cond, currentdiag,
						XCLRelationType.contradicted);
			}
			else if (weight.equals("!")) {
				XCLModel.insertXCLRelation(idom.getKnowledgeBase(), cond, currentdiag,
						XCLRelationType.requires);
			}
			else if (weight.equals("++")) {
				XCLModel.insertXCLRelation(idom.getKnowledgeBase(), cond, currentdiag,
				...
				XCLModel.insertXCLRelation(idom.getKnowledgeBase(), cond, currentdiag,
						XCLRelationType.explains, value);
			}
		}
		countfindings++;
	}
