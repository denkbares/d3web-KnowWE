...
public class D3SCMCBRBuilder implements SCMCBRBuilder, KnOfficeParser {
	...
	public List<Message> addKnowledge(Reader r,
		...
		DefaultLexer lexer = new DefaultLexer(istream, new DefaultD3webLexerErrorHandler(errors,
				file));
		lexer.setNewline(true);
		CommonTokenStream tokens = new CommonTokenStream(lexer);
		SCMCBR parser = new SCMCBR(tokens, this, new DefaultD3webParserErrorHandler(errors, file,
				"BasicLexer"));
		try {
			parser.knowledge();
		}
	...
	public void solution(int line, String text, String name) {
		...
		if (currentdiag == null) {
			errors.add(MessageKnOfficeGenerator.createSolutionNotFoundException(file, line, text,
					name));
		}
	}
	...
	private void finding(String weight) {
		...
		if (currentdiag != null) {
			...
			try {
				...
				// Tritt nur bei einem Parserfehler auf, dieser setzt die
				// Fehlermelung
				value = 1.0;
			}
			SCMCBRModel.insertSCMCBRRelation(idom.getKnowledgeBase(), cond, currentdiag, type,
					value);
	...
	public void question(int line, String text, String name) {
		...
		if (currentquestion == null) {
			errors.add(MessageKnOfficeGenerator.createQuestionNotFoundException(file, line, text,
					name));
		}
	}
	...
	public void questionclass(int line, String text, String name) {
		...
		if (currentqclass == null) {
			errors.add(MessageKnOfficeGenerator.createQuestionClassNotFoundException(file, line,
					text, name));
		}
	}
