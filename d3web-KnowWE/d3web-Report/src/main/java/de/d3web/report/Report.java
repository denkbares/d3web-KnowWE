...
public class Report {
	...
	public int add(Message message) {
		if (message != null) {
			...
			if (message.getMessageType() == Message.ERROR) errors.add(message);
			else if (message.getMessageType() == Message.WARNING) warnings.add(message);
			else if (message.getMessageType() == Message.NOTE) notes.add(message);
		}
	...
	public int addAll(List<Message> messages) {
		for (Iterator<Message> it = messages.iterator(); it.hasNext();) {
			...
			if (nextMessage.getMessageType() == Message.ERROR) errors.add(nextMessage);
			else if (nextMessage.getMessageType() == Message.WARNING) warnings.add(nextMessage);
			else if (nextMessage.getMessageType() == Message.NOTE) notes.add(nextMessage);
		}
		return allMessages.size();
	}
