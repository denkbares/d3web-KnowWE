...
public class TableColHeaderCellContentRenderer extends TableCellContentRenderer {
	...
	protected String wrappContent(String sectionText, Section sec, KnowWEUserContext user) {
		...
		KnowWEObjectType tableObjectType = TableUtils.getTableType(sec);
		Section father = sec.findAncestorOfType(TableHeaderLine.class);

		if (sort) {
			html.append("<th class=\"sort\">");
		}
		else if (father == null && TableUtils.getTableType(sec).toString().startsWith(
				"de.d3web.we.flow.testcase.TestcaseTable")) {
