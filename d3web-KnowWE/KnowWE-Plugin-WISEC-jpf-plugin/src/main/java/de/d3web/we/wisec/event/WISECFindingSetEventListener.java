...
public class WISECFindingSetEventListener implements EventListener<FindingSetEvent> {
	...
	public void notify(FindingSetEvent event, String web, String username, 
		...
		if (!checkQuestion(event.getQuestion()))
			return;
		
		// Specifies whether vales are added (true) or subtracted (false)
		boolean add = true;
		if (event.getValue().toString().equals("excluded"))
			add = false;
		...
		for (Criteria criteria : Criteria.values()) {
			...
			try {
				...
				Logger.getLogger(this.getClass()).warn("Unable to create SPARQL-Query, no values set! " + e.getMessage());
			} catch (MalformedQueryException e) {
				Logger.getLogger(this.getClass()).warn("Unable to create SPARQL-Query, no values set! " + e.getMessage());
			} catch (QueryEvaluationException e) {
				Logger.getLogger(this.getClass()).warn("Unable to evaluate SPARQL-Query, no values set! " + e.getMessage());
			}
	...
	/**
	 * Checks if the question which triggered this event is a question
	 * of the "Substances"-Questionnaire.
	 */
	...
	private TupleQueryResult evaluateQuery(Query query) throws QueryEvaluationException {
		if (!(query instanceof TupleQuery)) 
			throw new IllegalStateException("Query needs to be an instance of TupleQuery.");

		return ((TupleQuery) query).evaluate();
	}
	
	/**
	 * Computes the numerical value which will be added to the counter.
	 * The value is based on the committed TupleQueryResult.
	 */
	private double computeNumValue(Criteria criteria, TupleQueryResult result,
		...
		while (result.hasNext()) {
			...
			// TODO: Check this!
			if ((activeLists.contains(currentList) && add) // Don't add if list was already computed 
					|| (!activeLists.contains(currentList) && !add)) // Don't subtract if list hasn't been computed yet
				continue;
	...
	private void setCounterValue(Criteria criteria, double accumulatedValue,
		...
		if (kss == null) {
			Logger.getLogger(this.getClass()).error("Unable to get KnowledgeServiceSession for namespace: " + namespace);
			return;
		}
		...
		if (counterQuestion == null) {
			Logger.getLogger(this.getClass()).error("Counter-Question: " + criteria + " was not found! No value set!");
			return;
		}
