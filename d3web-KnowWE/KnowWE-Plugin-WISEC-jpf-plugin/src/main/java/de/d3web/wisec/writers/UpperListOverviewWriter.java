...
public class UpperListOverviewWriter extends WISECWriter {
	private static final String FILENAME = WISECExcelConverter.FILE_PRAEFIX + "InformationSources";
	
	private static final String[] PRINTABLE_ATTRIBUTES = new String[] {WISECExcelConverter.NUMBER_KEY, 
		"Inhalt", "Institution", "Pefix", "Incl"};
	...
	public void write() throws IOException {
		...
		for (UpperList list : model.getUpperLists()) {
			for (String attribute : PRINTABLE_ATTRIBUTES) {
				...
				else if (value.equals(list.getName())) {
					value = "["+value+" | " + UpperListWriter.getWikiFilename(value) + "]";
				}
