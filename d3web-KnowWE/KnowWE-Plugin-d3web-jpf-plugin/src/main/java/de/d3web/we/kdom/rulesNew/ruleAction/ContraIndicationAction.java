 ...
 * @created 26.07.2010
 */
public class ContraIndicationAction extends D3webRuleAction<ContraIndicationAction> {
	...
	public ContraIndicationAction() {
		this.sectionFinder = new ConditionalSectionFinder(new AllTextSectionFinder()) {
			...
			protected boolean condition(String text, Section<?> father) {
				if (text.startsWith("NICHT") || text.startsWith("NOT")) {
		...
		AnonymousType negKey = new AnonymousType("neg-key");
		negKey.setSectionFinder(new OneOfStringEnumUnquotedFinder(new String[] {
				"NICHT", "NOT" }));
		this.addChildType(negKey);
		
		this.addChildType(new QuestionReferenceInBrackets());
