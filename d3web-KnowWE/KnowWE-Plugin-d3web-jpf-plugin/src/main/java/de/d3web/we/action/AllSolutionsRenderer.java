...
public class AllSolutionsRenderer extends DeprecatedAbstractKnowWEAction {
	...
	public String perform(KnowWEParameterMap parameterMap) {
		...
		if (!unclear.isEmpty()) {
			...
			sb.append("<div style='padding:1em'>");
			sb.append("<a href=\"/bin/view/" + web + "/Unclear\"><b>"
					+ rb.getString("KnowWE.solution.unclearSolutions") + ":</b>");
			sb.append("</a>");
		...
		if (!excluded.isEmpty()) {
			...
			sb.append("<div style='padding:1em'>");
			sb.append("<a href=\"/bin/view/" + web + "/Excluded\"><b>"
		...
		if (!conflict.isEmpty()) {
			...
			sb.append("<div style='padding:1em'>");
			sb.append("<a href=\"/bin/view/" + web + "/Conflict\"><b>"
	...
	private StringBuffer getSolutionLinkList(Map<String, String> parameterMap, List<Term> list, ISetMap<Term, Information> assumptionMap) {
		...
		for (Term term : list) {
			...
			String exactPrefix = KnowWEUtils.replaceUmlaut(((String) term.getInfo(TermInfoType.TERM_NAME)))
					+ "buttomLink";
			sb.append("<a href=\"javascript:doNothing()\" onclick=\"showPopupButtons('"
					+ exactPrefix + "', event)\">");
			...
			inner.append(KnowWERenderUtils.getExplanationLinks(user, web, term, iconURL, "dps",
					true, true));
			inner.append(KnowWERenderUtils.getClarificationLinks(user, web, term, iconURL, "dps",
					true, true));
			inner.append(KnowWERenderUtils.getSolutionLogLinks(user, web, term, iconURL, true, true));
			inner.append(KnowWERenderUtils.getDialogLinks(user, web, term, iconURL, "dps", true,
					true));
