...
public class GenerateKBAction extends DeprecatedAbstractKnowWEAction {
	...
	public String perform(KnowWEParameterMap parameterMap) {
		...
		String kbString = KnowWEEnvironment.getInstance().getWikiConnector().getAttachmentPath(
				parameterMap.get(KnowWEAttributes.ATTACHMENT_NAME));
		...
		if (KnowWEEnvironment.getInstance().getWikiConnector().doesPageExist(
				parameterMap.get(KnowWEAttributes.NEWKB_NAME))) {
			try {
				if (this.appendKnowledgeBase(KnowWEEnvironment.getInstance(),
						parameterMap.getWeb(), parameterMap.get(KnowWEAttributes.NEWKB_NAME),
						kbString, parameterMap.getUser())) {
		...
		try {
			...
			if (testMap != null) {
				String updateContent = KnowWEEnvironment.getInstance().getWikiConnector().createWikiPage(
						parameterMap.get(KnowWEAttributes.NEWKB_NAME), testMap,
						parameterMap.getUser());
				KnowWEEnvironment.getInstance().processAndUpdateArticle(parameterMap.getUser(),
						updateContent, parameterMap.get(KnowWEAttributes.NEWKB_NAME),
						parameterMap.getWeb());
				// Link zu neuer page...
				return "<p class='info box'>"
	...
	/**
	 * Reads all the Knowledge from a jar. Returns null if an error occurred or
	 * the jar was no KB.
	 * 
	 * @param kbString
	 * @throws IOException
	...
	/**
	 * Checks if an Article has a Kopic Module in it. If no: Appends the KB to
	 * the Article.
	 * 
	 * @param env
	 * @param web
