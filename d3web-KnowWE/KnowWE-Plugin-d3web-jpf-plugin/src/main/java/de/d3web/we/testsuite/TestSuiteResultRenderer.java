 ...
 * @created 25/10/2010
 */
public class TestSuiteResultRenderer extends DefaultMarkupRenderer<TestSuiteResultType> {

	private ResourceBundle rb;
	private MessageFormat formatter;

	public TestSuiteResultRenderer() {
		super("KnowWEExtension/d3web/icon/testsuite24.png");
	}

	@Override
	protected void renderContents(KnowWEArticle article, Section<TestSuiteResultType> section, KnowWEUserContext user, StringBuilder string) {

		this.rb = D3webModule.getKwikiBundle_d3web(user);
		this.formatter = new MessageFormat("");

		string.append(mask("<strong>" + TestSuiteResultType.getText(section) + "</strong><br />\n"));

		String testSuiteArticleName = TestSuiteResultType.getTestSuite(section);
