 ...
 * @author jochen
 * 
 *         Type for the coveringTable-solutionHeaderLine. The first Line
 *         contains solutions.
 * 
 */
public class SolutionTableHeaderLine extends TableHeaderLine {
	...
	class SolutionHeaderLineSubtreeHandler extends SubtreeHandler {
		...
		/*
		 * creates the solution contexts allowing the EntryCells (@see
		 * EntryCellContent) to easily access their column context. (@see
		 * SolutionColumnContext)
		 */
		@Override
		public Collection<KDOMReportMessage> create(KnowWEArticle article, Section s) {
			...
			for (Section<TableCell> section : cells) {
				...
				if (findChildOfType != null) {
					...
					if (solution.trim().length() == 0) solution = null;
				}
