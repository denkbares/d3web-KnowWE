 ...
 */
public class DeployAttachmentSnapshotProvider implements ToolProvider {

	private static final Logger LOGGER = LoggerFactory.getLogger(DeployAttachmentSnapshotProvider.class);
	...
	private @NotNull List<Tool> createAllSnapshotTools(@NotNull Section<?> section) {
		...
		return attachmentsZips.stream()
				.map(att -> createTool(att.getFileName(), section))
				.toList();
	}

	public static @NotNull List<WikiAttachment> getWikiAttachmentSnapshots() {
		...
		try {
			...
			LOGGER.error("Could not retrieve attachments list" );
	...
	private @NotNull Tool createTool(@NotNull String filename, @NotNull Section<?> section) {
		...
		String jsAction = "const userConfirmed = confirm('" + message + "');" +
				"if (userConfirmed) {" +
				"	window.location = 'action/" + DeployAttachmentSnapshotAction.class.getSimpleName() + "?" + KEY_DEPLOY_FILENAME + "=" + filename + "&amp;" + Attributes.TOPIC + "=" + section.getTitle() + "'; " +
				"}";
