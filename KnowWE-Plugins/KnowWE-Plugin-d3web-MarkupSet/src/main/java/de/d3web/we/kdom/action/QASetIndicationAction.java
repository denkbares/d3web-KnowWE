		...
		public void compile(D3webCompiler compiler, Section<AnonymousType> s) throws CompilerMessage {
			...
			if (terminologyHandler.isDefinedTerm(termIdentifier)) {
				...
				if (termDefinitionSection.get() instanceof Term) {
					...
					Messages.storeMessage(compiler, s, this.getClass(),
							Messages.error(Strings.trimQuotes(termIdentifier.toPrettyPrint())
										   + "is defined as: " + objectClazz.getName()
										   + " - expected was Question or Questionnaire"));
					return;
				}
			}
			Messages.storeMessage(compiler, s, this.getClass(),
					Messages.error("Could not find '"
								   + Strings.trimQuotes(termIdentifier.toPrettyPrint())
								   + "' - expected was Question or Questionnaire"));
		}

		@Override
		public void destroy(D3webCompiler compiler, Section<AnonymousType> section) {
			Messages.clearMessages(compiler, section, this.getClass());
		}

		@Override
		public boolean isIncrementalCompilationSupported(Section<AnonymousType> section) {
			return true;
		}
	}
}
