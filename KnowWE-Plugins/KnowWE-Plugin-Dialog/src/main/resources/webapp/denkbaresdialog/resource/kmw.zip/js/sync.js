...
Sync.startUpdate = function () {
	var url = "/KnowWE/action/StartUpdate/"+(new Date().getTime());
...
Sync.cancelUpdate = function () {
	var url = "/KnowWE/action/CancelUpdate/"+(new Date().getTime());
	new Ajax.Request(url, {
		method: 'get',
		onSuccess: function(transport) {
		},
...
Sync.checkProgress = function () {
	var url = "/KnowWE/action/CheckProgress/"+(new Date().getTime());
	new Ajax.Request(url, {
		method: 'get',
		onSuccess: function(transport) {
			if (Sync.checkProgressTaskStarted) {
