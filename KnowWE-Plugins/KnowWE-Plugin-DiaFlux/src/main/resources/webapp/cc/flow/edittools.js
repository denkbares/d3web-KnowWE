...
FlowEditor.EditorToolMenu.prototype.initTools = function() {
	...
	var format = function(flowEditor) {
		var xml = flowEditor.theFlowchart.toXML();

		new Ajax.Request("KnowWE.jsp", {
			method : 'post',
			parameters : {
				action : 'DotFormatterAction',
				KWikitext : xml,				// content
			},
			onSuccess : function(transport) {
				flowEditor.withUndo("Format", function() {
					flowEditor._restoreData(transport.responseText, flowEditor._getMetaData());
				});
			},
			onFailure : function() {
				CCMessage.warn(
					'AJAX Error',
					'Flow could not be formatted.');
			},
			onException : function(transport, exception) {
				CCMessage.warn(
					'AJAX Error, Formatting most likely failed.',
					exception
				);
			}
		});
	...
	this.editTools = [
		...
		new FlowEditor.EditTool("Format", format, null),
