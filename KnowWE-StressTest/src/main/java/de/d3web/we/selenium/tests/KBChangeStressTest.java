...
public class KBChangeStressTest extends StressTestCase {
	...
	final long testDuration = Long.parseLong(bundle.getString("KnowWE.SeleniumStressTest.duration"));
	...
	private void stressChangeKnowledge(String url) {
		...
		while (System.currentTimeMillis() - startTime < testDuration) {
			try {
				...
				se.printStackTrace();
				errByChange = " (see console for SeleniumException caused by the changing actions)";
				break;
			}
		}
		// Check page and wiki loading (still working or down)
		try {
			open(url);
		}
		catch (SeleniumException se) {
			try {
				...
				fail("Main-Page load timed out (whole server down)" + errByChange + ".");
			}
			fail("KB-Page load timed out (this page is down)" + errByChange + ".");
		}
	}
