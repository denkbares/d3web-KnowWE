...
/**
 * Abstract Class containing some special methods being helpful
 * to test knowledge functionality like questionsheet or dialog.
 * @author Max Diez
 *
 */
public abstract class KnowledgeTestCase extends KnowWETestCase{
	 ...
	 * @param isDialog If true the dialog is checked, otherwise the questionsheet
	...
	protected boolean checkAndUncheckSolutions(String[] expSolutions, String[] notExpSolutions,
		...
		if (isDialog) {
					...
					if (selenium.getText("//table[@id='qPage']").contains(elem)) {
						...
						for (int j = 1; !elemFound && j < 10000; j++) {
							if (selenium.isElementPresent(QPAGE_LOC + CAT_LOC + j + "']") &&
									selenium.getText(QPAGE_LOC + CAT_LOC + j + "']").contains(elem.trim())){
								//Choose all answers given in the int array of input
								for (int k = 0; k < input.get(elem).length; k++) {
									doSelActionAndWait(QPAGE_LOC + CAT_LOC + j + "']//input[@id='Q" + j + "a" + input.get(elem)[k] + "']", "click");
									doSelActionAndWait(QPAGE_LOC + CAT_LOC + j + "']" + CAT_OK, "click");									
								}
								elemFound = true;
								...
								for (int k = 0; k < input.get(elem).length; k++) {
									allSelected &= selenium.isChecked(CAT_LOC + j + "']//input[@id='Q" + j + "a" + input.get(elem)[k] + "']");
								}
								if (!allSelected) {
									elemFound = false;
									j--;
			...
			for (int j = 0; j < elem.length; j++) {
				...
				//Open the category's interview; it's recommended to wait until the frame pops up
				...
				//Counts the tries for setting the input for the actual category
				//stop if this counter is >= MAXLOOP
				int loopRepeatCounter = 0;
				//Iterating over all options of the actual category which should be chosen
				for (int i = 0; i < actCategoryInput.length && loopRepeatCounter < MAXLOOP; i++) {
					...
					if (selenium.isElementPresent(actInputLocator)) {
						...
						if (!selenium.isElementPresent(actInputLocator) 
							...
							//Something went wrong, so repeat this iteration step
							i--;
							loopRepeatCounter++;
							continue;
						}
					...
					//If interview is still opened, the input wasn't correct -> cancel test
